...
export class RemoteDbEntity {
	...
	get(param: any, orderby: string) {
		...
		return httpClientPool.httpClient.send(url, param).then((response: any) => {
	...
	find(param: any, orderby: string, from: number, limit: number, count: number) {
		...
		if (limit > 0) {
			params = { from: from, limit: limit, orderby: orderby, count: count, condiBean: param }
		}
		return httpClientPool.httpClient.send('/' + this.name + '/Find', params).then((response: any) => {
	...
	add(data: any[], selected: any[]): any {
		let index = this.index++
		let record = { state: EntityState.New, entityId: this.name + index }
		Vue.set(data, data.length, record) //data.push(record)
		if (selected) {
			selected.splice(0, selected.length)
			Vue.set(selected, 0, record) //selected.push(record)
		}
	...
	insert(data: any[]) {
		...
		if (stateData.length > 0) {
			return httpClientPool.httpClient.send('/' + this.name + '/Insert', stateData).then((response: any) => {
				const resData = response.data
				this.responseData(resData, data, dataMap)
	...
	update(data: any[]) {
		let { dataMap, stateData } = this.buildDataMap(data, false)
		if (stateData.length > 0) {
			return httpClientPool.httpClient.send('/' + this.name + '/Update', stateData).then((response: any) => {
				const resData = response.data
				this.responseData(resData, data, dataMap)
	...
	delete(data: any[]) {
		return httpClientPool.httpClient.send('/' + this.name + '/Delete', data).then((response: any) => {
			const resData = response.data
			if (resData && resData.length > 0) {
	...
	onQuasarPage(props: any, pagination: any, fn: any) {
	...
	prepareQuasarPage(pagination: any, queryChanged: boolean) {
		let count = pagination.rowsNumber
		if (queryChanged === true) {
			count = 0
		}
		let from = (pagination.page - 1) * pagination.rowsPerPage
		...
		return { orderby: orderby, from: from, limit: limit, count: count }
	}
}
